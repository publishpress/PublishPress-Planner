...
class PP_Calendar extends PP_Module {
	...
	function view_calendar() {
		...
		<div class="wrap">
			<div id="ef-calendar-title"><!-- Calendar Title -->
				...
				<h2><?php _e( 'Calendar', 'publishpress' ); ?>&nbsp;<span class="time-range"><?php $this->calendar_time_range(); ?></span></h2>
			</div><!-- /Calendar Title -->
			...
			<table id="ef-calendar-view" class="<?php echo esc_attr( implode( ' ', $table_classes ) ); ?>">
				...
				<td class="<?php echo esc_attr( implode( ' ', $td_classes ) ); ?>" id="<?php echo esc_attr( $week_single_date ); ?>">
					...
					<?php if ( $week_single_date == date( 'Y-m-d', current_time( 'timestamp' ) ) ): ?>
						<div class="day-unit-today"><?php _e( 'Today', 'publishpress' ); ?></div>
					<?php endif; ?>
					...
					?>
						...
						<form method="POST" class="post-insert-dialog">
							...
							<h1><?php echo sprintf( __( 'Schedule a %1$s for %2$s', 'publishpress' ), $this->get_quick_create_post_type_name(), $date_formatted ); ?></h1>
							...
							<div class="post-insert-dialog-controls">
								...
								<a class="post-insert-dialog-edit-post-link" href="#"><?php echo esc_html( sprintf( _x( 'Edit %s', 'post type name', 'publishpress' ), $this->get_quick_create_post_type_name() ) ); ?>&nbsp;&raquo;</a>
							</div>
	 ...
	 * @since 0.8
	 */
	function get_inner_information( $pp_calendar_item_information_fields, $post ) {
		?>
				...
				<?php foreach( $this->get_post_information_fields( $post ) as $field => $values ): ?>
					<tr class="item-field item-information-<?php echo esc_attr( $field ); ?>">
						...
						<?php elseif( $values['value'] ): ?>
							<td class="value"><?php echo esc_html( $values['value'] ); ?></td>
						<?php else: ?>
						<td class="value"><em class="none"><?php echo _e( 'None', 'publishpress' ); ?></em></td>
						<?php endif; ?>
					</tr>
				<?php endforeach; ?>
	...
	function print_top_navigation( $filters, $dates ) {
		...
		<ul class="ef-calendar-navigation">
			...
			<li class="date-change next-week">
				<a title="<?php printf( __( 'Forward 1 week', 'publishpress' ) ); ?>" href="<?php echo esc_url( $this->get_pagination_link( 'next', $filters, 1 ) ); ?>"><?php _e( '&rsaquo;', 'publishpress' ); ?></a>
				<?php if ( $this->total_weeks > 1): ?>
				<a title="<?php printf( __( 'Forward %d weeks', 'publishpress' ), $this->total_weeks ); ?>" href="<?php echo esc_url( $this->get_pagination_link( 'next', $filters ) ); ?>"><?php _e( '&raquo;', 'publishpress' ); ?></a>
				<?php endif; ?>
			</li>
			<li class="date-change today">
				<a title="<?php printf( __( 'Today is %s', 'publishpress' ), date( get_option( 'date_format' ), current_time( 'timestamp' ) ) ); ?>" href="<?php echo esc_url( $this->get_pagination_link( 'next', $filters, 0 ) ); ?>"><?php _e( 'Today', 'publishpress' ); ?></a>
			</li>
			<li class="date-change previous-week">
				<?php if ( $this->total_weeks > 1): ?>
				<a title="<?php printf( __( 'Back %d weeks', 'publishpress' ), $this->total_weeks ); ?>"  href="<?php echo esc_url( $this->get_pagination_link( 'previous', $filters ) ); ?>"><?php _e( '&laquo;', 'publishpress' ); ?></a>
				<?php endif; ?>
				<a title="<?php printf( __( 'Back 1 week', 'publishpress' ) ); ?>" href="<?php echo esc_url( $this->get_pagination_link( 'previous', $filters, 1 ) ); ?>"><?php _e( '&lsaquo;', 'publishpress' ); ?></a>
			</li>
			...
			case 'type':
				...
				?>
					<select id="type" name="cpt">
						<option value=""><?php _e( 'View all types', 'publishpress' ); ?></option>
					<?php
